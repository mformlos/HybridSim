...
inline void MPC::stream(MPCParticle& part, double dt) {
...
void MPC::sort() {
    ...
    for (auto& part : Fluid) {
        int CellIndex {}; 
        CellIndex = (int)part.Position(0) + (int)part.Position(1)*BoxSize[0] + (int)part.Position(2)*BoxSize[0]*BoxSize[1]; 
        try {
            CellList.at(CellIndex).push_front(&part); 
        }
        catch (std::exception& e) {
            std::cout << "The following exception occurred : " << e.what() << std::endl;
            std::cout << "for a particle with position "  << part.Position.transpose() << " and cell index " << CellIndex << std::endl;
...
void MPC::sortVector() {
    std::sort(Fluid.begin(), Fluid.end(), *this); 
}
