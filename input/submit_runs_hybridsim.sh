...
#copy it to scratch
cd ..
cp bin/hybridsim_auto /scratch/formanek/HYBRIDSIM
...
while read rundir; do 
    echo $rundir
    jobname="$(echo $rundir | sed 's/\/scratch\/formanek\/HYBRIDSIM\/runs\///')"
    echo $jobname
    echo $currentNode
    ...
    sed -i "s/NCPUS/$TasksPerNode/" slurm/$jobname.slurm  
