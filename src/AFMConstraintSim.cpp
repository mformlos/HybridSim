...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Anchortime is " << AnchorTime << std::endl;
    ...
    System Sys(Lx, Ly, Lz, 0.0, 0.0, AdsorptionOn, false); 
    ...
    */
    ...
    for (n = 0; n <= TotalSteps; n++, m++) {
        ...
        if (!Equilibrated && Time >= EquilTime) {
            Sys.SurfaceEnergy = SurfaceEStart; 
            std::cout << "System equilibrated, turning on surface adsorption" << std::endl;
            double mindist{Lz}; 
            Vector3d NewCOM {Sys.Molecules[0].centerOfMassPosition()};
            for (auto& mono : Sys.Molecules[0].Monomers) {
                if (mono.Position(2) < mindist) mindist = mono.Position(2);
        ...
        if (!Anchored && Time > AnchorTime) {
            if (Sys.Molecules[0].Monomers[0].Position(2) <= 1.05 && Sys.Molecules[0].Monomers[0].Position(2) >= 0.9 ) {
                std::cout << "trying to anchor " << std::endl;
                Sys.setAnchorAuto(0); 
